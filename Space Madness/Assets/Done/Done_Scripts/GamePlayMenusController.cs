...
public class GamePlayMenusController : MonoBehaviour
{
    ...
    public GameObject PromotionBig;
    private bool _isPromotionBigShown = false;
    ...
    void ShowPromotion()
    {
        if (_isPromotionBigShown == false & GameObject.FindGameObjectsWithTag("PromotionBig").Length == 0)
        {
            Instantiate(PromotionBig, Vector3.zero, Quaternion.identity);
            _isPromotionBigShown = true;
        }
    }

    void DestroyPromotion()
    {
        if (_isPromotionBigShown == true)
            _isPromotionBigShown = false;
        if (GameObject.FindGameObjectsWithTag("PromotionBig").Length > 0)
            Destroy(GameObject.FindGameObjectWithTag("PromotionBig"), 2);
    }
}
